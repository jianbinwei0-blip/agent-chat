...
class TestAgentChatControlPlane(unittest.TestCase):
    ...
    def test_parse_inbound_command_resume_with_bot_mention_prefix(self) -> None:
        cmd = cp._parse_inbound_command("@agentchat_bot @019c33b4 apply patch")  # type: ignore[attr-defined]
        self.assertEqual(cmd.get("action"), "resume")
        self.assertEqual(cmd.get("session_ref"), "019c33b4")
        self.assertEqual(cmd.get("prompt"), "apply patch")

    def test_parse_inbound_command_help_with_bot_mention_prefix(self) -> None:
        cmd = cp._parse_inbound_command("@agentchat_bot help")  # type: ignore[attr-defined]
        self.assertEqual(cmd.get("action"), "help")

    def test_parse_inbound_command_help_with_slash_command(self) -> None:
    ...
    def test_parse_inbound_command_resume_with_short_slash_without_at_prefix(self) -> None:
        cmd = cp._parse_inbound_command("/r 019c33b4 apply patch")  # type: ignore[attr-defined]
        self.assertEqual(cmd.get("action"), "resume")
        self.assertEqual(cmd.get("session_ref"), "019c33b4")
        self.assertEqual(cmd.get("prompt"), "apply patch")
    ...
    def test_bind_telegram_thread_to_session_rebinds_session_to_single_topic(self) -> None:
        registry: dict[str, object] = {
            "sessions": {
                "sid-1": {
                    "session_id": "sid-1",
                    "telegram_chat_id": "123456",
                    "telegram_message_thread_id": 111,
                }
            },
        ...
        cp._bind_telegram_thread_to_session(  # type: ignore[attr-defined]
            registry=registry,
            chat_id="123456",
        ...
        self.assertEqual(bindings, {"123456:222": "sid-1"})

        sessions = registry.get("sessions")
        self.assertIsInstance(sessions, dict)
        if not isinstance(sessions, dict):
            self.fail("expected sessions map")
        rec = sessions.get("sid-1")
        self.assertIsInstance(rec, dict)
        if not isinstance(rec, dict):
            self.fail("expected sid-1 record")
        self.assertEqual(rec.get("telegram_chat_id"), "123456")
        self.assertEqual(rec.get("telegram_message_thread_id"), 222)

    def test_bind_telegram_thread_to_session_topic_takeover_clears_old_session_metadata(self) -> None:
        registry: dict[str, object] = {
            "sessions": {
                "sid-1": {
                    "session_id": "sid-1",
                ...
                "sid-2": {
                    "session_id": "sid-2",
                    "telegram_chat_id": "123456",
                    "telegram_message_thread_id": 222,
                },
            },
            "telegram_thread_bindings": {
                "123456:111": "sid-1",
        ...
        if not isinstance(bindings, dict):
            self.fail("expected telegram_thread_bindings map")
        self.assertEqual(bindings, {"123456:111": "sid-2"})
        ...
        sid1 = sessions.get("sid-1")
        self.assertIsInstance(sid1, dict)
        if not isinstance(sid1, dict):
            self.fail("expected sid-1 record")
        self.assertIsNone(sid1.get("telegram_chat_id"))
        self.assertIsNone(sid1.get("telegram_message_thread_id"))

        sid2 = sessions.get("sid-2")
        self.assertIsInstance(sid2, dict)
        if not isinstance(sid2, dict):
            self.fail("expected sid-2 record")
        self.assertEqual(sid2.get("telegram_chat_id"), "123456")
        self.assertEqual(sid2.get("telegram_message_thread_id"), 111)

    def test_load_registry_strict_migration_enforces_single_topic_per_session(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            codex_home = Path(td)
        ...
        bindings = loaded.get("telegram_thread_bindings")
        self.assertIsInstance(bindings, dict)
        if not isinstance(bindings, dict):
            self.fail("expected telegram_thread_bindings map")
        self.assertEqual(
        ...
        sid_a = sessions.get("sid-a")
        self.assertIsInstance(sid_a, dict)
        if not isinstance(sid_a, dict):
            self.fail("expected sid-a record")
        self.assertEqual(sid_a.get("telegram_chat_id"), "123456")
        self.assertEqual(sid_a.get("telegram_message_thread_id"), 20)

        sid_b = sessions.get("sid-b")
        self.assertIsInstance(sid_b, dict)
        if not isinstance(sid_b, dict):
            self.fail("expected sid-b record")
        self.assertEqual(sid_b.get("telegram_chat_id"), "123456")
        self.assertEqual(sid_b.get("telegram_message_thread_id"), 40)

        sid_c = sessions.get("sid-c")
        self.assertIsInstance(sid_c, dict)
        if not isinstance(sid_c, dict):
            self.fail("expected sid-c record")
        self.assertEqual(sid_c.get("telegram_chat_id"), "999999")
        self.assertEqual(sid_c.get("telegram_message_thread_id"), 30)

    def test_telegram_thread_id_for_session_prefers_mapping(self) -> None:
        registry = {
            "sessions": {
                "sid-123": {
                    "session_id": "sid-123",
    ...
    def test_telegram_thread_id_for_session_falls_back_to_session_metadata(self) -> None:
        registry = {
            "sessions": {
                "sid-123": {
                    "session_id": "sid-123",
    ...
    def test_handle_notify_payload_completion_origin_scoped_suppresses_discord_delivery_for_local_prompt(self) -> None:
        payload = {
            "type": "agent-turn-complete",
            "session_id": "sid-123",
            "last-assistant-message": "done",
        ...
        registry: dict[str, object] = {
            "sessions": {
                "sid-123": {
                    ...
                    "agent": "pi",
                    "discord_channel_id": "chan-123",
                    "discord_progress_mode": "origin_scoped",
                }
            }
        }
        sent: list[dict[str, object]] = []
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with (
                mock.patch.object(cp, "_load_registry", return_value=registry),
                mock.patch.object(cp, "_load_message_index", return_value={}),
                mock.patch.object(cp, "_save_registry"),
                mock.patch.object(cp, "_save_message_index"),
                mock.patch.object(cp, "_send_structured", side_effect=_capture_send),
                mock.patch.object(cp.agent_chat_dedupe, "mark_once", return_value=True),
        ...
        rec = registry["sessions"]["sid-123"]  # type: ignore[index]
        self.assertEqual(rec.get("active_prompt_status"), "completed")
        self.assertEqual(rec.get("desktop_attention_state"), "resolved")

    def test_handle_notify_payload_completion_origin_scoped_keeps_discord_delivery_for_discord_prompt(self) -> None:
        payload = {
            "type": "agent-turn-complete",
            "session_id": "sid-123",
            "last-assistant-message": "done",
    ...
    def test_handle_notify_payload_failed_updates_lifecycle_and_attention(self) -> None:
        payload = {
            "type": "agent-turn-complete",
            "session_id": "sid-123",
            "status": "failed",
        ...
        self.assertEqual(sent[0].get("kind"), "failed")
        self.assertEqual(sent[0].get("discord_lifecycle_event"), True)
        rec = registry["sessions"]["sid-123"]  # type: ignore[index]
        self.assertEqual(rec.get("active_prompt_status"), "failed")
        self.assertEqual(rec.get("desktop_attention_state"), "resolved")

    def test_handle_notify_payload_cancelled_updates_lifecycle_and_attention(self) -> None:
        payload = {
            "type": "agent-turn-complete",
            "session_id": "sid-123",
            "reason": "cancelled",
        ...
        self.assertEqual(sent[0].get("kind"), "cancelled")
        self.assertEqual(sent[0].get("discord_lifecycle_event"), True)
        rec = registry["sessions"]["sid-123"]  # type: ignore[index]
        self.assertEqual(rec.get("active_prompt_status"), "cancelled")
        self.assertEqual(rec.get("desktop_attention_state"), "resolved")
    ...
    def test_persist_attention_state_updates_registry_and_files(self) -> None:
        sid = "11111111-1111-1111-1111-111111111111"
        with tempfile.TemporaryDirectory() as td:
            codex_home = Path(td)
            registry: dict[str, object] = {
            ...
            with mock.patch.object(cp.time, "time", return_value=1234):
                cp._persist_attention_state(  # type: ignore[attr-defined]
                    codex_home=codex_home,
                    registry=registry,
                    session_id=sid,
                    session_rec=rec,
            ...
            self.assertEqual(rec.get("desktop_attention_state"), "notification_visible")
            self.assertEqual(rec.get("last_desktop_attention_ts"), 1234)
            self.assertEqual(rec.get("tmux_pane"), "%9")
            self.assertEqual(rec.get("tmux_socket"), "/tmp/tmux-501/default")

            last_state = cp._load_last_attention_state(codex_home=codex_home)  # type: ignore[attr-defined]
            attention_index = cp._load_attention_index(codex_home=codex_home)  # type: ignore[attr-defined]
            self.assertIsInstance(last_state, dict)
            self.assertIsInstance(attention_index, dict)
            if not isinstance(last_state, dict) or not isinstance(attention_index, dict):
                self.fail("expected persisted attention state files")
            self.assertEqual(last_state.get("session_id"), sid)
            self.assertEqual(last_state.get("desktop_attention_state"), "notification_visible")
            self.assertEqual(last_state.get("tmux_pane"), "%9")
            indexed = attention_index.get(sid)
            self.assertIsInstance(indexed, dict)
            if not isinstance(indexed, dict):
                self.fail("expected per-session attention record")
            self.assertEqual(indexed.get("desktop_attention_state"), "notification_visible")

    def test_emit_desktop_visibility_for_discord_prompt_inline_prefers_tmux_banner(self) -> None:
        sid = "11111111-1111-1111-1111-111111111111"
        with tempfile.TemporaryDirectory() as td:
            codex_home = Path(td)
            registry: dict[str, object] = {
            ...
            with (
                mock.patch.object(cp, "_tmux_discover_codex_pane_for_session", return_value=("%9", "/tmp/tmux-501/default")),
                mock.patch.object(cp, "_tmux_desktop_presence_for_pane", return_value="foreground_attached"),
                mock.patch.object(cp, "_tmux_show_desktop_visibility_banner", return_value=True) as banner_mock,
                mock.patch.object(cp, "_show_macos_desktop_notification", return_value=False) as notify_mock,
                mock.patch.object(cp.time, "time", return_value=2000),
            ):
                state = cp._emit_desktop_visibility_for_discord_prompt(  # type: ignore[attr-defined]
                    codex_home=codex_home,
                    registry=registry,
                    session_id=sid,
                    session_rec=rec,
                    prompt="please continue with the desktop-visible task",
                    agent="pi",
                )

            self.assertEqual(state, "inline_visible")
            self.assertEqual(rec.get("tmux_pane"), "%9")
            self.assertEqual(rec.get("tmux_socket"), "/tmp/tmux-501/default")
            banner_mock.assert_called_once()
            notify_mock.assert_not_called()
            self.assertEqual(rec.get("desktop_attention_state"), "inline_visible")

    def test_emit_desktop_visibility_for_discord_prompt_background_uses_notification(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with (
                mock.patch.object(cp, "_tmux_pane_exists", return_value=True),
                mock.patch.object(cp, "_tmux_pane_matches_session", return_value=True),
                mock.patch.object(cp, "_tmux_desktop_presence_for_pane", return_value="background_attached"),
                mock.patch.object(cp, "_tmux_show_desktop_visibility_banner", return_value=True) as banner_mock,
                mock.patch.object(cp, "_show_macos_desktop_notification", return_value=True) as notify_mock,
                mock.patch.object(cp.time, "time", return_value=3000),
            ):
                state = cp._emit_desktop_visibility_for_discord_prompt(  # type: ignore[attr-defined]
                    codex_home=codex_home,
                    registry=registry,
                    session_id=sid,
                    session_rec=rec,
                    prompt="investigate the failing Discord visibility path",
                    agent="pi",
                )

            self.assertEqual(state, "notification_visible")
            banner_mock.assert_not_called()
            notify_mock.assert_called_once()
            self.assertEqual(rec.get("desktop_attention_state"), "notification_visible")

    def test_process_inbound_replies_discord_pi_accepted_mentions_desktop_visibility(self) -> None:
        sid = "sid-pi"
        registry: dict[str, object] = {
            "sessions": {
                sid: {
        ...
        with (
            ...
            mock.patch.object(cp.reply, "_fetch_new_replies", return_value=[(501, "continue", None)]),
            ...
            mock.patch.object(cp, "_emit_desktop_visibility_for_discord_prompt", return_value="inline_visible") as visibility_mock,
            mock.patch.object(cp, "_send_structured", side_effect=_capture_send),
            mock.patch.object(cp, "_save_registry"),
            mock.patch.object(cp, "_save_message_index"),
        ):
        ...
        self.assertEqual(rowid, 501)
        visibility_mock.assert_called_once()
        self.assertTrue(
            any(
                msg.get("kind") == "accepted"
                and msg.get("session_id") == sid
                and "visible on the desktop" in str(msg.get("text", "")).lower()
                for msg in sent
            )
        )
    ...
    def test_process_inbound_replies_tmux_failed_strict_marks_prompt_failed_and_resolves_attention(self) -> None:
        sid = "019c33b4-e0ed-7021-940a-02b1e8147a82"
        registry = {
            "sessions": {
                sid: {
        ...
        self.assertEqual(rowid, 101)
        rec = registry["sessions"][sid]  # type: ignore[index]
        self.assertEqual(rec.get("active_prompt_status"), "failed")
        self.assertEqual(rec.get("desktop_attention_state"), "resolved")
        self.assertTrue(any(m.get("kind") == "error" for m in sent))
        self.assertTrue(any(m.get("kind") == "failed" and m.get("discord_lifecycle_event") for m in sent))
    ...
    def test_process_inbound_replies_implicit_telegram_thread_binding_resolves_target_first(self) -> None:
        ...
        with (
            sqlite3.connect(":memory:") as conn,
            mock.patch.object(cp.reply, "_fetch_new_replies", return_value=[(301, "continue", None)]),
            mock.patch.object(cp.reply, "_is_attention_message", return_value=False),
            mock.patch.object(cp.reply, "_is_bot_message", return_value=False),
            mock.patch.object(cp, "_load_registry", return_value=registry),
            mock.patch.object(cp, "_load_message_index", return_value=message_index),
            ...
            mock.patch.object(cp, "_resolve_session_agent", return_value="codex"),
            mock.patch.object(cp, "_dispatch_prompt_to_session", return_value=("resume", "done")),
            mock.patch.object(cp, "_send_structured", side_effect=_capture_send),
            mock.patch.object(cp, "_save_registry"),
            mock.patch.object(cp, "_save_message_index"),
        ):
    ...
    def test_discord_progress_mode_helpers_normalize_and_gate_by_origin(self) -> None:
        ...
        self.assertFalse(
            cp._should_emit_discord_lifecycle_event(  # type: ignore[attr-defined]
                session_rec={"discord_progress_mode": "origin_scoped"},
        ...
        self.assertTrue(
            cp._should_emit_discord_lifecycle_event(  # type: ignore[attr-defined]
                session_rec={"discord_progress_mode": "shared_status"},
                event_kind="completed",
            )
        )

    def test_bind_discord_context_to_session_sets_default_progress_mode_and_preserves_explicit_mode(self) -> None:
        registry: dict[str, object] = {
            "sessions": {
                "sid-a": {"session_id": "sid-a", "agent": "pi"},
                "sid-b": {
                    "session_id": "sid-b",
                    "agent": "pi",
        ...
        cp._bind_discord_context_to_session(  # type: ignore[attr-defined]
            ...
            discord_parent_channel_id="chan-a",
        )
        cp._bind_discord_context_to_session(  # type: ignore[attr-defined]
            registry=registry,
            session_id="sid-b",
            discord_channel_id="chan-b",
    ...
    def test_update_active_prompt_lifecycle_sets_attention_defaults(self) -> None:
        ...
        with mock.patch.object(cp.time, "time", return_value=777):
            cp._update_active_prompt_lifecycle(  # type: ignore[attr-defined]
                registry=registry,
                session_id="sid-1",
                status="needs_input",
            )
        rec = registry["sessions"]["sid-1"]  # type: ignore[index]
        self.assertEqual(rec.get("active_prompt_status"), "needs_input")
        self.assertEqual(rec.get("desktop_attention_state"), "waiting_for_user")
        self.assertEqual(rec.get("last_desktop_attention_ts"), 777)

        with mock.patch.object(cp.time, "time", return_value=888):
            cp._update_active_prompt_lifecycle(  # type: ignore[attr-defined]
                registry=registry,
                session_id="sid-1",
                status="cancelled",
        ...
        self.assertEqual(rec.get("active_prompt_status"), "cancelled")
        self.assertEqual(rec.get("desktop_attention_state"), "resolved")
        self.assertEqual(rec.get("last_desktop_attention_ts"), 888)

    def test_process_inbound_replies_discord_prompt_records_active_prompt_metadata_for_pi_session(self) -> None:
        registry: dict[str, object] = {
            "sessions": {
                "sid-discord": {
                    "session_id": "sid-discord",
        ...
        with (
            ...
            mock.patch.object(cp, "_emit_desktop_visibility_for_discord_prompt", return_value="notification_visible"),
        ):
            rowid = cp._process_inbound_replies(  # type: ignore[attr-defined]
                conn=conn,
                after_rowid=0,
        ...
        resolve_context_mock.assert_not_called()
        rec = registry["sessions"]["sid-discord"]  # type: ignore[index]
        self.assertEqual(rec.get("active_prompt_origin"), "discord")
    ...
    def test_process_inbound_replies_implicit_telegram_infers_single_bound_thread_when_missing(self) -> None:
        ...
        with (
            sqlite3.connect(":memory:") as conn,
            mock.patch.object(cp.reply, "_fetch_new_replies", return_value=[(312, "hello", None)]),
            mock.patch.object(cp.reply, "_is_attention_message", return_value=False),
            mock.patch.object(cp.reply, "_is_bot_message", return_value=False),
            mock.patch.object(cp, "_load_registry", return_value=registry),
            mock.patch.object(cp, "_load_message_index", return_value=message_index),
        ...
        self.assertEqual(rowid, 312)
        resolve_context_mock.assert_not_called()
        self.assertTrue(
            any(
                msg.get("session_id") == "sid-123"
                and msg.get("telegram_message_thread_id") == 888
                for msg in sent
            )
        )
    ...
    def test_process_inbound_replies_implicit_telegram_owner_auto_binds_single_chat_session(self) -> None:
        ...
        def _row_contexts(*, conn: sqlite3.Connection, after_rowid: int, handle_ids: list[str]) -> dict[int, dict[str, object]]:
            ...
            return {
                311: {
                    ...
                    "telegram_chat_id": "123456",
                    "telegram_message_thread_id": 999,
                    "telegram_sender_user_id": "8780500517",
                }
            }

        with (
            sqlite3.connect(":memory:") as conn,
            mock.patch.object(cp.reply, "_fetch_new_replies", return_value=[(311, "hello after patch", None)]),
            mock.patch.object(cp.reply, "_is_attention_message", return_value=False),
            mock.patch.object(cp.reply, "_is_bot_message", return_value=False),
            mock.patch.object(cp, "_load_registry", return_value=registry),
            mock.patch.object(cp, "_load_message_index", return_value=message_index),
            ...
            mock.patch.dict(
                ...
                {
                    "AGENT_CHAT_STRICT_TMUX": "1",
                    "AGENT_TELEGRAM_OWNER_USER_IDS": "8780500517",
                },
                clear=False,
            ),
        ):
        ...
        self.assertEqual(rowid, 311)
        resolve_context_mock.assert_not_called()
        bindings = registry.get("telegram_thread_bindings")
        self.assertIsInstance(bindings, dict)
        if not isinstance(bindings, dict):
            self.fail("expected telegram_thread_bindings map")
        self.assertEqual(bindings.get("123456:999"), "sid-123")
        self.assertTrue(
            any(
                msg.get("kind") == "responded"
                and msg.get("session_id") == "sid-123"
                and msg.get("telegram_message_thread_id") == 999
                for msg in sent
            )
        )

    def test_process_inbound_replies_unresolved_telegram_thread_sets_thread_scoped_pending_choice(self) -> None:
        registry: dict[str, object] = {
            "sessions": {},
            "pending_new_session_choice": None,
            "pending_new_session_choice_by_thread": {},
        ...
        with (
            sqlite3.connect(":memory:") as conn,
            mock.patch.object(cp.reply, "_fetch_new_replies", return_value=[(302, "continue", None)]),
            mock.patch.object(cp.reply, "_is_attention_message", return_value=False),
            mock.patch.object(cp.reply, "_is_bot_message", return_value=False),
            mock.patch.object(cp, "_load_registry", return_value=registry),
            mock.patch.object(cp, "_load_message_index", return_value=message_index),
        ...
        self.assertEqual(rowid, 302)
        self.assertIsNone(registry.get("pending_new_session_choice"))
        pending_by_thread = registry.get("pending_new_session_choice_by_thread")
        self.assertIsInstance(pending_by_thread, dict)
        if not isinstance(pending_by_thread, dict):
            self.fail("expected pending_new_session_choice_by_thread map")
        pending = pending_by_thread.get("123456:888")
        self.assertIsInstance(pending, dict)
        if not isinstance(pending, dict):
            self.fail("expected thread-scoped pending payload")
        self.assertEqual(pending.get("prompt"), "continue")
        self.assertTrue(
            any(
                msg.get("kind") == "needs_input" and msg.get("telegram_message_thread_id") == 888
                for msg in sent
            )
        )

    def test_process_inbound_replies_thread_scoped_choice_binds_created_session(self) -> None:
        registry: dict[str, object] = {
            "sessions": {},
            "pending_new_session_choice": None,
            "pending_new_session_choice_by_thread": {
        ...
        def _fake_create_new_session_in_tmux(**kwargs: object):
            captured.update(kwargs)
            return "sid-999", "/tmp/sessions/sid-999.jsonl", "%7", None
        ...
        with (
            sqlite3.connect(":memory:") as conn,
            mock.patch.object(cp.reply, "_fetch_new_replies", return_value=[(303, "2", None)]),
            mock.patch.object(cp.reply, "_is_attention_message", return_value=False),
            mock.patch.object(cp.reply, "_is_bot_message", return_value=False),
            mock.patch.object(cp, "_load_registry", return_value=registry),
            mock.patch.object(cp, "_load_message_index", return_value=message_index),
        ...
        if not isinstance(pending_by_thread, dict):
            self.fail("expected pending_new_session_choice_by_thread map")
        self.assertNotIn("123456:888", pending_by_thread)
        bindings = registry.get("telegram_thread_bindings")
        self.assertIsInstance(bindings, dict)
        if not isinstance(bindings, dict):
            self.fail("expected telegram_thread_bindings map")
        self.assertEqual(bindings.get("123456:888"), "sid-999")
        self.assertTrue(
            any(
                msg.get("kind") == "accepted"
                and msg.get("session_id") == "sid-999"
                and msg.get("telegram_message_thread_id") == 888
                for msg in sent
            )
        )
    ...
    def test_main_once_telegram_transport_skips_chat_db_open(self) -> None:
        with (
            mock.patch.dict(
                cp.os.environ,
                {
            ...
            mock.patch.object(cp, "_acquire_single_instance_lock", return_value=object()),
            mock.patch.object(cp, "_load_outbound_cursor", return_value=({}, {})),
            mock.patch.object(cp, "_open_chat_db") as open_db_mock,
            mock.patch.object(cp, "_process_outbound", return_value=({}, {})),
            mock.patch.object(cp, "_save_outbound_cursor"),
            mock.patch.object(cp, "_process_inbound_replies") as inbound_mock,
            mock.patch.object(cp, "_save_inbound_cursor") as save_inbound_mock,
            mock.patch.object(cp, "_load_telegram_inbound_cursor", return_value=10),
            mock.patch.object(cp, "_process_inbound_telegram_replies", return_value=10) as telegram_inbound_mock,
            mock.patch.object(cp, "_save_telegram_inbound_cursor") as save_telegram_cursor_mock,
        ):
            rc = cp.main(["once"])
        ...
        inbound_mock.assert_not_called()
        save_inbound_mock.assert_not_called()
        telegram_inbound_mock.assert_called_once()
        save_telegram_cursor_mock.assert_not_called()
    ...
    def test_doctor_report_telegram_transport_ignores_chat_db_warning(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            codex_home = Path(td)
            tmp_dir = codex_home / "tmp"
            tmp_dir.mkdir(parents=True, exist_ok=True)
        ...
        self.assertEqual(report["transport"]["mode"], "telegram")  # type: ignore[index]
        self.assertEqual(report["launchd"]["inbound_warning"], True)  # type: ignore[index]
        self.assertNotIn(  # type: ignore[arg-type]
            "launchd reports inbound disabled (check chat.db permissions)",
            report.get("issues", []),
    ...
    def test_should_emit_discord_lifecycle_event_respects_mode_and_origin(self) -> None:
        self.assertTrue(
            cp._should_emit_discord_lifecycle_event(  # type: ignore[attr-defined]
                session_rec={"discord_progress_mode": "origin_scoped", "active_prompt_origin": "discord"},
        ...
        self.assertFalse(
            cp._should_emit_discord_lifecycle_event(  # type: ignore[attr-defined]
                session_rec={"discord_progress_mode": "origin_scoped", "active_prompt_origin": "desktop"},
                event_kind="responded",
            )
        )
        self.assertTrue(
    ...
    def test_send_structured_formats_discord_lifecycle_needs_input_copy(self) -> None:
        ...
        def _capture_discord(*, token: str, channel_id: str, message: str) -> bool:
            del token, channel_id
            sent_discord.append(message)
            return True
        ...
        with (
            ...
            mock.patch.object(cp, "_load_registry", return_value=registry),
            mock.patch.object(cp, "_send_discord_message", side_effect=_capture_discord),
            mock.patch.object(cp.outbound, "_send_imessage"),
        ):
            cp._send_structured(  # type: ignore[attr-defined]
                codex_home=Path(td),
                recipient="discord-recipient",
        ...
        self.assertEqual(len(sent_discord), 1)
        self.assertIn("Pi is waiting for your input on `@abcd1234`.", sent_discord[0])
        self.assertIn("1) Which fix should Pi apply?", sent_discord[0])
        self.assertIn("Reply here to continue.", sent_discord[0])
        self.assertIn("Try: `1`, `2`, `summarize`, or your own instructions.", sent_discord[0])
        self.assertNotIn("[Pi] sid-pi — needs_input", sent_discord[0])

    def test_send_structured_formats_discord_lifecycle_completed_copy(self) -> None:
        sent_discord: list[str] = []
        registry = {
            "sessions": {
                "sid-pi": {
        ...
        self.assertEqual(sent_discord, ["Done in `@abcd1234`.\nImplemented the requested fix."])

    def test_send_structured_formats_discord_lifecycle_accepted_inline_visible_copy(self) -> None:
        sent_discord: list[str] = []
        registry = {
            "sessions": {
                "sid-pi": {
                    ...
                    "desktop_attention_state": "inline_visible",
                    "discord_progress_mode": "origin_scoped",
                }
            },
            "conversation_bindings": {},
        ...
        with (
            ...
            cp._send_structured(  # type: ignore[attr-defined]
                ...
                session_id="sid-pi",
                kind="accepted",
                text="caller-specific text should not leak to discord",
                max_message_chars=1800,
                dry_run=False,
                message_index={},
                discord_lifecycle_event=True,
        ...
        self.assertEqual(sent_discord, ["Got it — sent to `@abcd1234`. It's now visible on the desktop."])

    def test_send_structured_formats_discord_lifecycle_accepted_attention_copy(self) -> None:
        sent_discord: list[str] = []
        registry = {
            "sessions": {
                "sid-pi": {
                    ...
                    "active_prompt_origin": "discord",
                    "active_prompt_status": "accepted",
                    "desktop_attention_state": "notification_visible",
                    "discord_progress_mode": "origin_scoped",
                }
            },
            "conversation_bindings": {},
        ...
        self.assertEqual(sent_discord, ["Got it — sent to `@abcd1234`. It's queued in the session and marked for desktop attention."])

    def test_send_structured_formats_discord_lifecycle_failed_and_cancelled_copy(self) -> None:
        sent_discord: list[str] = []
        registry = {
            "sessions": {
                "sid-pi": {
    ...
    def test_process_session_file_emits_working_then_completed_for_discord_origin_prompt(self) -> None:
        sent: list[dict[str, object]] = []
        registry = {
            "sessions": {
                "sid-pi": {
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            session_path.write_text(
                '{"type":"message","message":{"role":"assistant","content":"Completed summary"}}\n',
                encoding="utf-8",
            )
            with (
                mock.patch.dict(
        ...
        self.assertEqual([entry.get("kind") for entry in sent], ["working", "responded"])
        self.assertEqual(sent[0].get("deliver_to_imessage"), False)
        self.assertEqual(sent[0].get("deliver_to_telegram"), False)
        self.assertEqual(sent[0].get("deliver_to_discord"), True)
        self.assertEqual(sent[0].get("discord_lifecycle_event"), True)
        self.assertEqual(sent[1].get("deliver_to_discord"), True)
        self.assertEqual(sent[1].get("discord_lifecycle_event"), True)
        rec = registry["sessions"]["sid-pi"]  # type: ignore[index]
        self.assertEqual(rec.get("active_prompt_status"), "completed")
        self.assertEqual(rec.get("desktop_attention_state"), "resolved")
        self.assertFalse(rec.get("pending_completion"))

    def test_process_session_file_suppresses_discord_completion_for_origin_scoped_desktop_prompt(self) -> None:
        sent: list[dict[str, object]] = []
        registry = {
            "sessions": {
                "sid-pi": {
        ...
        self.assertEqual([entry.get("kind") for entry in sent], ["responded"])
        self.assertEqual(sent[0].get("deliver_to_discord"), False)
    ...
    def test_send_structured_uses_telegram_transport_when_enabled(self) -> None:
        ...
        self.assertIsNone(sent_telegram[0][3])

    def test_send_structured_uses_bound_telegram_thread_for_session(self) -> None:
        sent_telegram: list[tuple[str, str, str, int | None]] = []
        ...
        registry = {
            ...
            "telegram_thread_bindings": {"123456:321": "sid-123"},
        }
        ...
        with (
            ...
            mock.patch.dict(
                cp.os.environ,  # type: ignore[attr-defined]
                {
                    "AGENT_CHAT_TRANSPORT": "telegram",
                    "AGENT_TELEGRAM_BOT_TOKEN": "test-bot-token",
                    "AGENT_TELEGRAM_CHAT_ID": "-1003836591224",
                },
                clear=False,
            ),
            mock.patch.object(cp, "_send_telegram_message", side_effect=_capture_telegram),
            ...
            cp._send_structured(  # type: ignore[attr-defined]
                ...
                dry_run=False,
                message_index={},
                telegram_chat_id="8780500517",
            )

        self.assertEqual(imessage_send.call_count, 0)
        self.assertGreaterEqual(len(sent_telegram), 1)
        self.assertEqual(sent_telegram[0][1], "8780500517")
        self.assertIsNone(sent_telegram[0][3])
    ...
    def test_fetch_telegram_updates_parses_text_messages(self) -> None:
        ...
        self.assertEqual(updates[0][4], "123456")

    def test_fetch_telegram_updates_accepts_multiple_chat_ids(self) -> None:
        class _FakeHTTPResponse:
            def __init__(self, body: str) -> None:
                self._body = body.encode("utf-8")
        ...
        with mock.patch.object(
            ...
            updates = cp._fetch_telegram_updates(  # type: ignore[attr-defined]
                token="test-bot-token",
                chat_ids=["123456", "8780500517"],
                after_update_id=0,
            )

        self.assertEqual(len(updates), 3)
        self.assertEqual([entry[0] for entry in updates], [100, 101, 102])
        self.assertEqual([entry[1] for entry in updates], ["from-group", "from-dm", ""])
        self.assertEqual([entry[4] for entry in updates], ["123456", "8780500517", "999999"])

    def test_fetch_telegram_updates_keeps_non_text_updates_for_cursor_progress(self) -> None:
        class _FakeHTTPResponse:
            def __init__(self, body: str) -> None:
                self._body = body.encode("utf-8")
        ...
        self.assertEqual(len(updates), 1)
        self.assertEqual(updates[0][0], 110)
        self.assertEqual(updates[0][1], "")
        self.assertEqual(updates[0][3], 46)
        self.assertEqual(updates[0][4], "123456")

    def test_fetch_telegram_updates_accepts_channel_post_payloads(self) -> None:
        class _FakeHTTPResponse:
            def __init__(self, body: str) -> None:
                self._body = body.encode("utf-8")
            ...
            "result": [
                {
                    ...
                    "channel_post": {
                        ...
                        "text": "@019caf68-33fc-7932-895c-728d7cfdc2bb ping",
                    },
                }
            ],
        }
        ...
        self.assertEqual(len(updates), 1)
        self.assertEqual(updates[0][0], 120)
        self.assertEqual(updates[0][1], "@019caf68-33fc-7932-895c-728d7cfdc2bb ping")
        self.assertEqual(updates[0][3], 47)
        self.assertEqual(updates[0][4], "123456")

    def test_fetch_telegram_updates_accepts_edited_message_payloads(self) -> None:
        class _FakeHTTPResponse:
            def __init__(self, body: str) -> None:
                self._body = body.encode("utf-8")
            ...
            "result": [
                {
                    ...
                    "edited_message": {
                        "chat": {"id": 123456},
                        "message_thread_id": 47,
                        "text": "@019caf68-33fc-7932-895c-728d7cfdc2bb edited ping",
                    },
                }
            ],
        }
        ...
        self.assertEqual(len(updates), 1)
        self.assertEqual(updates[0][0], 121)
        self.assertEqual(updates[0][1], "@019caf68-33fc-7932-895c-728d7cfdc2bb edited ping")
        self.assertEqual(updates[0][3], 47)
        self.assertEqual(updates[0][4], "123456")

    def test_fetch_telegram_updates_advances_cursor_on_disallowed_chat(self) -> None:
        class _FakeHTTPResponse:
            def __init__(self, body: str) -> None:
                self._body = body.encode("utf-8")
        ...
        self.assertEqual(len(updates), 1)
        self.assertEqual(updates[0][0], 130)
        self.assertEqual(updates[0][1], "")
        self.assertEqual(updates[0][4], "999999")
    ...
    def test_fetch_telegram_updates_captures_message_thread_id(self) -> None:
        class _FakeHTTPResponse:
            def __init__(self, body: str) -> None:
                self._body = body.encode("utf-8")
        ...
        self.assertEqual(len(updates), 1)
        self.assertEqual(updates[0][0], 62)
        self.assertEqual(updates[0][3], 9876)

    def test_fetch_telegram_updates_captures_sender_user_id(self) -> None:
        class _FakeHTTPResponse:
            def __init__(self, body: str) -> None:
                self._body = body.encode("utf-8")
            ...
            "result": [
                {
                    ...
                    "message": {
                        "chat": {"id": 123456},
                        "from": {"id": 8780500517, "is_bot": False},
                        "text": "continue",
                    },
                }
            ],
        ...
        self.assertEqual(len(updates), 1)
        self.assertEqual(updates[0][0], 63)
        self.assertEqual(updates[0][5], "8780500517")
    ...
    def test_process_inbound_telegram_replies_passes_reply_reference_texts(self) -> None:
        with (
            mock.patch.object(
                ...
                return_value=[(77, "continue", "[Codex] sid-123 — responded — done", 4321, "8780500517", "8780500517")],
            ),
            ...
            mock.patch.object(cp, "_telegram_chat_ids", return_value=["123456", "8780500517"]),
        ):
        ...
        row_contexts_fn = process_mock.call_args.kwargs.get("row_contexts_fn")
        self.assertTrue(callable(row_contexts_fn))
        if not callable(row_contexts_fn):
            self.fail("expected row_contexts_fn callback")
        probe_conn = sqlite3.connect(":memory:")
        try:
            row_contexts = row_contexts_fn(conn=probe_conn, after_rowid=0, handle_ids=[])
        finally:
            probe_conn.close()
        self.assertEqual(
            row_contexts.get(77),
            {
                "transport": "telegram",
                "telegram_chat_id": "8780500517",
                "telegram_message_thread_id": 4321,
                "telegram_sender_user_id": "8780500517",
            },
        )

    def test_process_inbound_telegram_replies_accepts_all_chats_when_enabled(self) -> None:
        with (
            mock.patch.object(cp, "_fetch_telegram_updates", return_value=[] ) as fetch_mock,
            mock.patch.object(cp, "_telegram_bot_token", return_value="token"),
            mock.patch.object(cp, "_telegram_chat_ids", return_value=["123456"]),
            mock.patch.dict(
                cp.os.environ,  # type: ignore[attr-defined]
                {"AGENT_TELEGRAM_ACCEPT_ALL_CHATS": "1"},
                clear=False,
            ),
        ):
            rowid = cp._process_inbound_telegram_replies(  # type: ignore[attr-defined]
        ...
        self.assertEqual(rowid, 55)
        fetch_mock.assert_called_once()
        self.assertIsNone(fetch_mock.call_args.kwargs.get("chat_ids"))
    ...
    def test_main_setup_permissions_skips_for_telegram_only_transport(self) -> None:
        with (
            mock.patch.dict(
                cp.os.environ,
                {
    ...
    def test_run_setup_launchd_bootstraps_missing_telegram_chat_id(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            home = Path(td)
            codex_home = home / ".codex"
            script_path = home / "repo" / "agent_chat_control_plane.py"
            ...
            with (
                ...
                mock.patch.object(
                    cp,
                    "_bootstrap_telegram_group_chat_id",
                    return_value=("-1003836591224", None),
                ) as bootstrap_mock,
                mock.patch.object(cp.subprocess, "run", return_value=mock.Mock(returncode=0, stderr="")),
                mock.patch("sys.stdout", new_callable=io.StringIO),
            ):
            ...
            bootstrap_mock.assert_called_once_with(
                codex_home=codex_home,
                timeout_s=float(cp._DEFAULT_TELEGRAM_SETUP_BOOTSTRAP_TIMEOUT_S),  # type: ignore[attr-defined]
                open_link=False,
            )
            ...
            env_vars = payload.get("EnvironmentVariables")
            self.assertIsInstance(env_vars, dict)
            self.assertEqual((env_vars or {}).get("AGENT_TELEGRAM_CHAT_ID"), "-1003836591224")
            setup_mock.assert_not_called()

    def test_run_setup_launchd_telegram_transport_ignores_chat_db_warning(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            home = Path(td)
            codex_home = home / ".codex"
            script_path = home / "repo" / "agent_chat_control_plane.py"
    ...
    def test_run_setup_launchd_fails_when_bootstrap_cannot_resolve_telegram_chat_id(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with (
                ...
                mock.patch.object(cp, "_bootstrap_telegram_group_chat_id", return_value=(None, "timed out")),
                mock.patch("sys.stdout", new_callable=io.StringIO) as out,
            ):
                rc = cp._run_setup_launchd(  # type: ignore[attr-defined]
                    codex_home=codex_home,
            ...
            self.assertEqual(rc, 1)
            text = out.getvalue()
            self.assertIn("AGENT_TELEGRAM_CHAT_ID is required", text)
